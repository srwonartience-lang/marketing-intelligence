...
| **알아둘 것** | 신규 기능·베타·출시. 다음 캠페인 기획 때 검토할 소식 | 한 줄 목록 |
...
1. **판정 한 줄** — "오늘 확인해야 할 업데이트 사항이 N건 있습니다." (N = 조치 필요 + 알아둘 것) 또는 "오늘 확인할 업데이트 사항이 없습니다."
   - **조치 필요**: 기존 캠페인·계정 설정을 바꿔야 하는 변경 (지원 종료, 기능 제거, 필수 요건 등)
   - **알아둘 것**: 신규 기능·베타·출시 등 다음 캠페인 기획 때 검토할 소식
   - **참고**: 특정 플랫폼 조치와 무관한 업계 동향·리서치·주간 정리
