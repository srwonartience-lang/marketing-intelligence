...
1. **판정 한 줄** — "조치가 필요한 변경 N건" 또는 "오늘 확인할 변경이 없습니다"
...
- **갱신 지연 표시**: 마지막 수집이 36시간을 넘으면 상단에 "갱신 지연"이 표시됩니다. 하루 1회 수집 구조라 화면이 **실시간이 아니라는 점**을 푸터에 명시했습니다.
...
`.github/workflows/collect.yml`이 매일 **KST 오전 6시(UTC 21:00)** 에 다음 순서로 자동 실행합니다:
...
실행 시각을 KST 09:00에서 06:00으로 옮겼습니다. 주 사용자가 출근해 화면을 보는 시각이 오전 9시인데, 같은 시각에 파이프라인이 돌면 화면을 볼 때 아직 갱신이 끝나 있지 않습니다.
...
- **이 서비스는 실시간이 아닙니다.** RSS 수집이 하루 1회(KST 06:00)만 돌기 때문에, 화면이 약속할 수 있는 것은 "매일 아침 갱신 · 최근 24시간"까지입니다. 실시간에 가깝게 하려면 공식 소스(Google/Meta 공식 블로그 등)만 3~6시간 간격 cron을 추가로 돌리는 편이 정확합니다. 있지도 않은 실시간성을 문구로 약속하면, 사용자가 이 화면을 믿고 확인을 끝내는 순간 제품이 거짓말을 하게 됩니다.
