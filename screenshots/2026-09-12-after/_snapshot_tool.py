...
def fmt(d):                                   # 개선 후 JS와 동일 (다른 해면 연도 표기)
    dt = datetime.date(*map(int, d.split('-')))
    y = f"{dt.year}년 " if dt.year != THIS_YEAR else ""
    return f"{y}{dt.month}월 {dt.day}일 ({DOW[(dt.weekday()+1)%7]})"
...
# --- feed (기본값: 최근 7일)
_l = datetime.date(*map(int, latest.split('-')))
cutoff = (_l - datetime.timedelta(days=6)).isoformat()
feed_items = [e for e in srt if e["date"] >= cutoff]
feed = ('<div class="feed-controls">'
        '<input id="feed-q" type="search" placeholder="제목 · 요약 · 매체 검색" aria-label="피드 검색">'
        '<div class="cat-filter" id="range-filter">'
        '<button class="cat-pill mono-btn all active" data-days="7">최근 7일</button>'
        '<button class="cat-pill mono-btn all" data-days="30">최근 30일</button>'
        '<button class="cat-pill mono-btn all" data-days="0">전체 기간</button>'
        '</div></div>'
        f'<p class="cat-count-label caption">최근 7일 · {len(feed_items)}건</p>'
        '<div class="list">' + "".join(card(e, False) for e in feed_items) + '</div>')
...
for d in range(1, days_in + 1):
    ...
    if items:
        cal += '<button type="button" class="%s" aria-pressed="%s">%d<span class="cnt">%d</span></button>' % (
            cls, "true" if ds == latest else "false", d, len(items))
    else:
        cal += '<div class="%s">%d</div>' % (cls, d)
...
views = {
 "01-feed": (tabbar("feed"), f'<div id="feed-view" class="view active">{feed}</div>'),
